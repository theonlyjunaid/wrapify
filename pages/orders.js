    ...
    return (
        <div className='w-full'>
            <div className=' mx-auto w-full px-3'>
                <h1 className='font-bold text-2xl px-1  py-4 pt-10 mx-auto w-[900px]'>My Orders</h1>
...
<div className='level3:flex text-center level3:text-justify  level3:justify-around gap-4 md:gap-10 w-full sm:w-auto'>
                                ...
                                <div className='level3:flex  gap-5 mt-4 sm:mt-0 sm:gap-3 sm:ml-14 w-full sm:w-auto justify-center '>
                                    <Link href={`/order?id=${item._id}`}>
                                        <div className='cursor-pointer border rounded-md px-5  level5:px-10 py-2 my-2 mx-auto level3:mx-0 level3:my-0 sm:p-2 bg-gray-50 hover:bg-gray-200 w-max'>
                                            View Order
                                        </div>
                                    </Link>
                                    <div className='cursor-pointer border rounded-md px-5 level5:px-10 py-2 sm:p-2 mx-auto level3:mx-0 bg-gray-50 hover:bg-gray-200 w-max'>
                                        View Invoice
                                    </div>
                                ...
                                Object.keys(item.products).map((key, index) => {
                                    ...
                                    return (
                                        <div className='border-t flex py-2' key={key}>
                                            <Link className='w-[40%] level4:w-[15%] bg-gray-50 flex items-center justify-center mx-4 rounded-lg' href={`/skin/${item.products[key].brand}/${item.products[key].name[0].split("(")[0]}`}>

                                                <Image src={item.products[key].img} width={100} height={100} className='w-auto  py-1 h-[100px] ' />
                                            </Link>
                                            <div className='w-[60%] level4:w-[85%] b'>
                                                ...
                                                <div className='flex gap-2 justify-between w-full pr-5 '>
                                                    ...
                                                    <div className='w-4/6 items-center  hidden level4:block'>
                                 ...
                                 {item.products[key].desc2 === " " ? <div>Whether it's the result of climate change or a nuclear winter, all-year snowfall is just around the corner. When that day comes, you'll need Arctic Camo if you want to continue hiding from your responsibilities.</div> : <div>{item.products[key].desc2}</div>}
                                                        ...
                                                        <Link href={`/skin/${item.products[key].brand}/${item.products[key].name[0].split("(")[0]}`} >
                                                            <div className='text-blue-600 font-semibold text-base  cursor-pointer flex mt-5 justify-end'>
                                                                View Product
                                                            </div></Link>
                                                    </div>
