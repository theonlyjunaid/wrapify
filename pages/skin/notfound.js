...
//  const notfound = () => {
//     const samsung = models.filter(model => model.label === 'Samsung Model')
//     const sam = samsung[0].option_values
//     console.log(sam)
//     const sams = sam.map(sam => sam.value)
//     console.log(sams)
...
const color = ({ products, addToCart }) => {
  // console.log(mobile)
  const [brand, setBrand] = useState('Apple Model')
  const [model, setModel] = useState('iphone 14')
  const [size, setSize] = useState('backfit')
const [sizeavail, setSizeavail] = useState('backfit')
  let itemi = models.filter(modele => modele.label === brand)
  let item = itemi[0]?.option_values
  let items = item?.map(item => item.value)
  // console.log(item)
      const samsung = models.filter(modele => modele.label === 'Samsung Model')
    const sam = samsung[0].option_values
    // console.log(sam)
    const sams = sam.map(sam => sam.value)
  ...
  useEffect(() => {
    ...
    models.map((item) => {
      if (item.label === brand) {
        // console.log(item.option_values)
      }
    })
console.log(sizeavail.toLowerCase().includes("no sides"))
  ...
  return (
    <div className='md:flex w-full lg:w-4/5 mx-auto min-h-[600px] max-w-[1280px] py-10'>
      <div className='xl:w-1/2  flex justify-center'>
        ...
        <Image src={products[0].img} width={360} height={360} alt='image' className='w-[220px] sm:w-auto h-auto' />
      </div>
      <div className=' mx-auto md:mx-0 sm:w-4/5 md:w-1/2 px-5 md:px-2 pt-10  '>
        <div className='mx-auto md:mx-0 sm:w-full md:w-full px-5 md:px-2 pt-10    '>
          ...
          <div className=' xl:flex'>
            ...
            <select onChange={(e) => {
              setBrand(e.target.value + " Model")
              setSizeavail(e.target.value)
        ...
        <div className='mx-auto md:mx-0 sm:w-full md:w-full max-w-[320px] px-5 md:px-2 pt-10    '>
          ...
          <div className=' xl:flex'>
            <div className='w-full border border-black rounded-md my-5'>
              ...
              {!sizeavail.toLowerCase().includes("no sides") &&   <div className={`h-[100px] md:h-[120px] flex  gap-3 cursor-pointer ${size === 'fullcover' ? 'bg-[#ffa825]' : ""}`} onClick={() => setSize('fullcover')}>
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ brand: 'nothing', name: 'nothing phone 1' }).lean();
